...
async function create(ID, Color, Size, Owner, AppraisedValue){
	try{
		const gateway = new Gateway();
		const wallet = await buildWallet(Wallets, walletPath);
		const ccp = buildCCPOrg1();
...
app.post('/create',function(req, res){
	...
	create(req.body.ID,req.body.Color, req.body.Size, req.body.Owner, req.body.AppraisedValue).then(result => {
...
app.post('/readrange',function(req, res){
	console.log('incoming post readrange request');
	
	readRange(req.body.left,req.body.right).then(result => {
		res.setHeader("Content-Type","application/json");
		res.send(JSON.stringify(result.toString()));
		//res.send(JSON.stringify({"data":result.toString()}));
	});
...
app.post('/transfer',function(req, res){
	console.log('incoming post transfer request');

	transfer(req.body.ID, req.body.newOwner).then(result => {
		if(result == true){
			res.sendStatus(200);
		}
		else{
...
app.post('/execquery',function(req, res){
	console.log('incoming post execQuery request');

	execQuery(req.body).then(result => {
		res.setHeader("Content-Type","application/json");
		res.send(JSON.stringify(result.toString()));
		//res.send(JSON.stringify({"data":result.toString()}));
	});
...
app.post('/execquerywithpage',function(req, res){
	console.log('incoming post execQuery request');

	execQuery(req.body.query,req.body.pageSize, req.body.bookmark).then(result => {
		res.setHeader("Content-Type","application/json");
		res.send(JSON.stringify(result.toString()));
	});
})
